...
const deleteGroup = (req, res) => {
    ...
    if (req.authenticated === true) {
        ...
        if (checkArr[0] === 0) {
            ...
            let array = []
            if (pool) {
                deleteGroupList(paramGroupIndex, paramUserIndex, function (err, result) {
                    if (err) {
                    ...
                    if (result) {
                        if (result[0]['run'] === 0) {
                            ...
                            let success = 0
                            let data = []
                            let message = '해당 group 의 owner user 만 group 을 삭제할 권한이 있습니다'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
...
const deleteGroupList = (groupIndex, userIndex, callback) => {
    console.log(`deleteGroupList 호출됨.`)
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('select deleteGroup (?, ?) as run', [groupIndex, userIndex], function (err, results) {
            conn.release()
