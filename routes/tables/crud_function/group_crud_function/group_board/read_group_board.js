...
const readGroupBoard = (req, res) => {
    ...
    if (req.authenticated === true) {
        ...
        if (checkArr[0] === 0) {
            ...
            let array = []
            if (pool) {
                findGroupBoard (paramGbTitle, paramGbContent, paramGbImportance, paramGbCreatedAt, paramGbUpdatedAt, paramGbAuthor, paramGbCode, paramGroupCode, function(err, result) {
                    if (err) {
                        console.log('group board 검색 중 에러 발생.')
                        let response = new Response('group board 검색 중 에러 발생', array, err)
                        res.send(response)
                    }
                    if (result) {
                        console.dir(result)
                        array.push(result)
                        let response = new Response('group board 검색 성공', array, null)
                        res.send(response)
                    } else {
                        array.push(result)
                        let response = new Response('group board 검색 실패', array, null)
                        res.send(response)
                    }
                })
            } else {
...
const findGroupBoard = (title, content, importance, created_at, updated_at, author, gb_code, group_code, callback) => {
    console.log(`findGroupBoard 호출됨.`)
    let checkObj = {
        "gb_title": title,
        "gb_content": content,
        "gb_importance": importance,
        "gb_created_at": created_at,
        "gb_updated_at": updated_at,
        "gb_author": author,
        "gb_code" : gb_code,
    ...
    pool.getConnection(function (err, conn) {
        ...
        if (newKeyArray.length > 0) {
            let exec = conn.query('select * from group_board where ' + string, function (err, result) {
                conn.release()
