...
const updateGroupBoard = (req, res) => {
    ...
    if (req.authenticated === true) {
        ...
        paramNewObj['gb_updated_at'] = new Date().toISOString().replace(/T..+/, '')

        let checkArr1 = checkKey(paramSearchObj, 'group_board_column')
        let checkArr2 = checkKey(paramNewObj, 'group_board_column')
        if (checkArr1[0] === 0 || checkArr2[0] === 0) {
            ...
            let array = []
            if (pool) {
                updateGroupBoardList(paramSearchObj, paramNewObj, function (err, result) {
                    if (err) {
                        console.log(`group board 업데이트 도중 에러 발생함.`)
                        let response = new Response('group board 업데이트 도중 에러 발생함.', array, err)
                        res.send(response)
                    }
                    if (result) {
                        console.dir(result)
                        array.push(result)
                        let response = new Response('group board 업데이트 함', array, null)
                        res.send(response)
                    } else {
                        array.push(result)
                        let response = new Response('group board 업데이트 안됨.', array, null)
                        res.send(response)
                    }
                })
            } else {
...
const updateGroupBoardList = (searchObj, newObj, callback) => {
    ...
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('update group_board set ? where ' + string, newObj, function (err, results) {
            conn.release()
