        ...
        if (checkArr[0] === 0) {
            ...
            if (pool) {
                if (paramMemberIndex !== undefined && paramUserIndex !== undefined && paramGroupIndex !== undefined) {
                    deleteMemberList(paramMemberIndex, paramUserIndex, paramGroupIndex, function (err, result) {
                        if (err) {
                            console.log(`member delete 도중 에러 발생함.`)
                            let code = 500 //Internal Server Error
                            let success = 0
                            let data = array
                            let message = 'member delete 중 error 발생함.'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                            ...
                            if (result[0]['run'] === 0) {
                                ...
                                let success = 0
                                let data = []
                                let message = 'member_index 는 member 를 delete 할 권한이 없습니다'
                                let error = err
                                let response = new Response(code, success, null, data, message, error)
                                res.send(response)
                                ...
                                let success = 0
                                let data = []
                                let message = 'owner 를 delete 할 수 없습니다'
                                let error = err
                                let response = new Response(code, success, null, data, message, error)
                                res.send(response)
...
const deleteMemberList = (memberIndex, userIndex, groupIndex, callback) => {
    console.log(`deleteMemberList 호출됨.`)
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('select deleteMember (?,?,?) as run', [memberIndex, userIndex, groupIndex], function (err, results) {
            conn.release()
