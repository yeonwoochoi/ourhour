...
const readGroup = (req, res) => {
    ...
    if(req.authenticated === true) {
        ...
        if (checkArr[0] === 0) {
            ...
            console.log(`요청 파라미터 : ${paramGroupIndex}, ${paramUserOwnerId}`)
            if (pool) {
                findGroup (paramGroupIndex, paramUserOwnerId, function(err, result) {
                    if (err) {
                        ...
                        if (result[0]['run'] === 0) {
                            ...
                            let code = 200 //OK
                            let success = 1
                            let data = syncConnection.query('select * from groupview where gb_index = ?', [paramGroupIndex])
...
const findGroup = (gbIndex, userIndex, callback) => {
    ...
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('select readGroup(?,?) as run', [gbIndex, userIndex] , function (err, result) {
            conn.release()
