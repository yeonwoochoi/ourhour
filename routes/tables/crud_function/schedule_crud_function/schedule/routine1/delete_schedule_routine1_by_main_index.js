...
const deleteScheduleRoutine1ByMainIndex = (req, res) => {
    console.log(`deleteScheduleRoutine1ByMainIndex 호출됨.`)
        ...
        if (checkArr[0] === 0) {
            ...
            if (pool) {
                ...
                if (paramMainIndex !== undefined && paramMainIndex !== null) {
                    ...
                    deleteScheduleRoutine1List(paramMainIndex, function (err, result) {
                        if (err) {
                            console.log('main index 로 schedule routine1 삭제 중 에러 발생함.')
                            let code = 500; //Internal Server Error
                            let success = 0
                            let data = array
                            let message = 'main index 로 schedule routine1 삭제 중 error 발생함.'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                            ...
                            let success = 1
                            let data = array
                            let message = 'main index 로 schedule routine1 삭제 성공.'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                        } else if (result.affectedRows === 0) {
                            ...
                            let success = 0
                            let data = array
                            let message = 'main index 로 schedule routine1 삭제 실패.'
                            let error = null
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                        }
...
const deleteScheduleRoutine1List = (index, callback) => {
    console.log(`deleteScheduleRoutine1List 호출됨.`)
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('call deleteRoutine1(?)', [index], function (err, results) {
            conn.release()
...
module.exports.deleteScheduleRoutine1ByMainIndex = deleteScheduleRoutine1ByMainIndex
