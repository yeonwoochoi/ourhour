...
const readScheduleRoutine1ByUserId = (req, res) => {
    console.log(`readScheduleRoutine1ByUserId 호출됨`)
    let array = []
        ...
        if (checkArr[0] === 0) {
            ...
            if (pool) {
                if (userIndex !== undefined && userIndex !== null) {
                    findScheduleRoutine1ByUserId(userIndex, function (err, result) {
                        if (err) {
                            console.log(`userIndex 값으로 schedule routine1 검색 중 error 발생`)
                            let code = 500 //Internal Server Error
                            let success = 0
                            let data = array
                            let message = 'userIndex 값으로 schedule routine1 검색 중 error 발생'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                        }
                        if (result.length > 0) {
                            ...
                            let success = 1
                            let data = result
                            let message = 'userIndex 값으로 schedule routine1 검색 성공.'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                            ...
                            let success = 0
                            let data = result
                            let message = 'userIndex 값을 schedule routine1 검색 실패.'
                            let error = null
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
                        }
...
const findScheduleRoutine1ByUserId = (user_index, callback) => {
    console.log(`findScheduleRoutine1ByUserId 호출됨`)
...
module.exports.readScheduleRoutine1ByUserId = readScheduleRoutine1ByUserId