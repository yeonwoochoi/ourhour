    ...
    if (req.authenticated === true) {
        ...
        for (let i = 0; i < memberArr.length; i++) {
            ...
            if (checkArr[0] === 0) {
                ...
                let array = []
                if (pool) {
                    addFriend(paramUserIndex, paramFriendIndex, function (err, result) {
                        if (err) {
                            console.log('friend 추가 중 에러 발생함')
                            let code = 500 //Internal Server Error
                            let success = 0
                            let data = array
                            let message = 'friend 추가 중 error 발생함.'
                            let error = err
                            let response = new Response(code, success, null, data, message, error)
                            res.send(response)
...
const addFriend = (user, friend, callback) => {
    ...
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('select addFriend (?,?) as run', [user, friend], function(err, result) {
            conn.release()
