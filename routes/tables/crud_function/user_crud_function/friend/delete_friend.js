...
const deleteFriend = (req, res) => {
    ...
    if (req.authenticated === true) {
        ...
        if (checkArr[0] === 0) {
            ...
            let array = []
            if (pool) {
                deleteFriendList(paramUserIndex, paramFriendIndex, function (err, result) {
                    if (err) {
                        console.log('friend 인증 중 에러 발생함')
                        let code = 500 //Internal Server Error
                        let success = 0
                        let data = array
                        let message = 'friend 삭제 중 error 발생함.'
                        let error = err
                        let response = new Response(code, success, null, data, message, error)
                        res.send(response)
...
const deleteFriendList = (user, friend, callback) => {
    console.log(`deleteFriendList 호출됨.`)
    pool.getConnection(function (err, conn) {
        ...
        let exec = conn.query('select deleteFriend (?,?) as run', [user, friend], function(err, result) {
            conn.release()
