...
const readAuthFriend = (req, res) => {
    console.log(`readAuthFriend 호출됨.`)
    if (req.authenticated === true) {
        ...
        if (checkArr[0] === 0) {
            ...
            if (pool) {
                findAuthFriend (paramUserIndex, function(err, result) {
                    if (err) {
                        console.log('friend 요청 인증 목록 검색 중 에러 발생.')
                        let code = 500 //Internal Server Error
                        let success = 0
                        let data = []
                        let message = 'friend 요청 인증 목록 검색 중 에러 발생.'
                        let error = err
                        let response = new Response(code, success, null, data, message, error)
                        res.send(response)
                    } else if (result) {
                        ...
                        let success = 1
                        let data = result
                        let message = 'friend 요청 인증 목록 검색 성공.'
                        let error = err
                        let response = new Response(code, success, null, data, message, error)
                        res.send(response)
                    } else {
                        ...
                        let success = 0
                        let data = []
                        let message = 'friend 요청 인증 목록 검색 실패.'
                        let error = null
                        let response = new Response(code, success, null, data, message, error)
                        res.send(response)
                    }
...
const findAuthFriend = (user, callback) => {
    console.log(`findAuthFriend 호출됨.`)
...
module.exports.readAuthFriend = readAuthFriend
